...
for x in allfightercodes:
    ...
    l = db.FighterNames.find({"fighterCode":fightercode},{"gender":1,  "_id":0}).distinct('gender'); gender = l[0]
    ...
    if weight == 125:
        output = (output / 3 ) + 40 + 12 - 3
    if weight == 135:
        output = (output / 3 ) + 40 + 11 - 4
    if weight == 145:
        output = (output / 3 ) + 40 + 9 - 4
    if weight == 155:
        output = (output / 3 ) + 40 + 15 - 5
    if weight == 170:
        output = (output / 3 ) + 40 + 13 - 6
    if weight == 185:
        output = (output / 3 ) + 40 + 13 - 7
    if weight == 205:
        output = (output / 3 ) + 40 + 12 - 8
    if weight == 265:
    ...
    if fightercode == 'CirylGaneBon Gamin':
        strikingGrade = strikingGrade - 4
    

    if grapplingGrade < 75:
        grapplingGrade = (grapplingGrade * .1) + grapplingGrade
        
    if changedweight == True:
        chinGrade = chinGrade - 10
    
    if weight != 265:
        fighterGrade = ((strikingGrade * 3.9) +
        (grapplingGrade * 11.0) +
        (defenseGrade * 2.2) +
        (chinGrade * 2.9) +
        (powerGrade * 1.5))
    ...
    y = db.WEBFighterData.find({"fighterCode":x}, {"LastFighterGrade":1, "_id":0}).distinct('LastFighterGrade')
    lastfightergrade = y[0]
    
    
    
    if lastfightergrade > 0:
        
        
        
        
        testfloat = float(lastfightergrade)
        lastfightergrade = round(testfloat, 0)
        
        
        
        
        testfloat = float(fighterGrade)
        fighterGrade = round(testfloat, 0)
        
        db.WEBFighterData.update_one({"fighterCode":x}, {"$set": {"FighterGrade": fighterGrade}})    
        db.WEBFighterData.update_one({"fighterCode":x}, {"$set": {"LastFighterGrade": lastfightergrade}})
        ...
        db.WEBFighterData.update_one({"fighterCode":x}, {"$set": {"LastFighterGrade": fighterGrade}})
        db.WEBFighterData.update_one({"fighterCode":x}, {"$set": {"FighterGrade": fighterGrade}})
    ...
    testfloat = float(strikingGrade)
    strikingGrade = strikingGrade -.5
    strikingGrade = round(strikingGrade, 0)
        
        
    
        
    
    testfloat = float(grapplingGrade)
    grapplingGrade = grapplingGrade -.5
    grapplingGrade = round(grapplingGrade, 0)
    
    
        
    
    testfloat = float(defenseGrade)
    defenseGrade = defenseGrade -.5
    defenseGrade = round(defenseGrade, 0)
    ...
    testfloat = float(chinGrade)
    chinGrade = chinGrade -.5
    chinGrade = round(chinGrade, 0)
    ...
    db.WEBFighterData.update_one({"fighterCode":x}, {"$set": {"FighterGrade": fighterGrade}})
    db.WEBFighterData.update_one({"fighterCode":x}, {"$set": {"StrikingGrade": strikingGrade }})
                                
    db.WEBFighterData.update_one({"fighterCode":x}, {"$set": {"GrapplingGrade": grapplingGrade}})
    
    db.WEBFighterData.update_one({"fighterCode":x}, {"$set": {"DefensiveGrade": defenseGrade}})
    
    db.WEBFighterData.update_one({"fighterCode":x}, {"$set": {"ChinGrade": chinGrade}})
